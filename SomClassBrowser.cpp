...
#include "SomClassBrowser.h"
#include "SomAstModel.h"
#include "SomParser.h"
#include "SomHighlighter.h"
...
public:
    ...
    {
        Ast::Expression* e = d_that->d_curMethod->findByPos( cur.position() + d_that->d_curMethod->d_loc.d_pos );
            ...
            case Ast::Thing::T_Ident:
                {
                    ...
                    cur.setPosition( offCorr( e->d_loc.d_pos - d_that->d_curMethod->d_loc.d_pos ) );
            ...
            case Ast::Thing::T_MsgSend:
                {
                    ...
                    {
                        cur.setPosition( offCorr( s->d_pattern[i].second.d_pos - d_that->d_curMethod->d_loc.d_pos ) );
...
bool ClassBrowser::parse(const QString& path, bool recursive)
{
    ...
    QStringList errors;
    Model::Files files;
    foreach( const QString& f, paths )
    {
        QFile* file = new QFile(f,this);
        if( !file->open(QIODevice::ReadOnly) )
            errors << tr("cannot open file for reading: %1").arg(f);
        else
            files.append( Model::File(file,f) );
    }
    const bool res = d_mdl->parse(files);
    qDebug() << "parsed in" << time.elapsed() << "ms";
    if( !res )
    {
        errors += d_mdl->getErrs();
    }
    foreach( const Model::File& f, files )
    {
        delete f.d_dev;
    }
    if( !errors.isEmpty() )
    {
        qCritical() << errors;
        QMessageBox::critical(this,tr("Parser Errors"), errors.join('\n'));
        return false;
    }
...
void ClassBrowser::fillMethod()
{
    ...
    QFile in(d_curMethod->d_loc.d_source);
...
void ClassBrowser::fillNamedUse(Ast::Named* n)
{
    ...
    while( i < ids.size() )
    {
        if( ids[i]->d_inMethod != m )
        {
            ...
            item->setData(0,Qt::UserRole,id->d_loc.d_pos);
            if( isAssig )
                item->setFont(0,bold);
...
void ClassBrowser::fillPatternUse(const QByteArray& pat)
{
    ...
    while( i < sends.size() )
    {
        if( sends[i]->d_inMethod != m )
        {
            ...
            item->setData(0,Qt::UserRole,send->d_pattern.first().second.d_pos );
...
void ClassBrowser::onUseClicked()
{
    ...
    if( !d_curMethod.isNull() )
    {
        ...
        if( pos > d_curMethod->d_loc.d_pos )
        {
            cur.setPosition( pos - d_curMethod->d_loc.d_pos );
